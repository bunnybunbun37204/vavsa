...
class BallSketch extends React.Component {
    ...
    Sketch = (p) => {
      ...
      class Balls {
        constructor(minEllipseX, maxEllipseX, ellipseStepAmp, ellipseStepSpeed, ellipseStepInit,
          ellipseStepD, ellipseStepTimeSpeed, ellipseY, noiseFreq, rotateAngle, col, col2, numBall, numFunction) {
          ...
          if (this.minEllipseX <= this.maxEllipseX) { this.ellipseDirection = 1; }
          else { this.ellipseDirection = -1; }
        }
        ...
        drawBalls() {
          ...
          for (let i = 0; i < this.numFunction; i++) {
            let theta = this.ellipseStepInit[i] + this.ellipseStepSpeed[i] * ellipseStepCount + this.ellipseStepTimeSpeed[i] * this.timeCount;
            val += this.ellipseStepAmp[i] * (p.sin(this.noiseFreq[i] * 2*p.PI * theta) * 0.5 + 0.5)**this.ellipseStepD[i];
          }
          val /= this.numFunction;
          
          let ellipseStepX = this.ellipseDirection * (_minW / 100 + val);
          
          let currentEllipseX = this.minEllipseX;// + random(ellipseStepX);
          let ellipseR = p.abs(ellipseStepX);
          p.fill(p.lerpColor(this.col, this.col2, p.sin(this.colorAmp * val)*0.5 + 0.5));
          ...
          if (this.ellipseDirection == 1) {
            for (let i = 0; i < this.numBall; i++) {
              ...
              ellipseStepX = this.ellipseDirection * (prevEllipseR + val);
        ...
        const setObject = (p) => {
          _minW = p.min(p.width, p.height) * 0.8;//1.15;
          ...
          for (let i = 0; i < numFunction; i++) {
            ellipseStepAmp.push(_minW / 51);//25 * random(0.5, 1.5));
            ellipseStepSpeed.push(0.04 * p.random(0.5, 1.9));//0.01 * random(0.7, 1.5));
            ellipseStepInit.push(p.random(1000));
            ellipseStepD.push(1);//random(2, 5));
            ellipseStepTimeSpeed.push(0.005 * p.random([1, -1]));//0.0015 * random([1, -1]));
            noiseFreq.push(2);//random(4,7));
          ...
          for (let i = 0; i < numBalls; i++) {
            let minEllipseX = -_minW/2;// * (-1)**i;
            let maxEllipseX = _minW/2;// * (-1)**i;
            let ellipseY = (-_minW / 2 + _minW / numBalls * (0.5 + i)) * yRatio;
            let rotateAngle = 0;//PI/2 + PI;//0;//PI/2;// * (-1)**i;
            ...
            _aryBalls.push(new Balls(minEllipseX, maxEllipseX, ellipseStepAmp, ellipseStepSpeed, ellipseStepInit2,
              ellipseStepD, ellipseStepTimeSpeed, ellipseY, noiseFreq, rotateAngle, col, col2, numBall, numFunction));
          }
        ...
        p.draw = () => {
          p.background(255);//60/100*255);//60);
          p.translate(p.width/2, p.height/2);
          let level = amplitude.getLevel(); 
          for (let i = 0; i < _aryBalls.length; i++) {
            let sizeFactor = p.map(level, 0, 1, 0.5, 2); 
            //_aryBalls[i].noiseFreq = level * 10;
            console.log(_aryBalls[i].noiseFreq);
            ...
            //console.log(_aryBalls[i].col.levels);
